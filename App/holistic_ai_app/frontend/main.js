...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function generateAiFormula() {
        ...
        DOMElements.formulaText.value = `GENEROWANIE RECEPTURY PRZEZ AI...\nTyp: ${productType}\nBaza: ${base}\nSkładniki Aktywne: ${activeIngredients.join(', ') || 'brak'}\nProszę czekać...`;
        DOMElements.formulaOutput.classList.remove('hidden');

        try {
            const response = await fetch('http://localhost:5000/api/ai/generate-formula', {
                method: 'POST',
                headers: {
            ...
            DOMElements.formulaText.value = data.formula || "Nie otrzymano receptury od AI.";

        } catch (error) {
            console.error("Błąd podczas generowania receptury AI:", error);
            DOMElements.formulaText.value = `Błąd: ${error.message}\nUpewnij się, że backend i Ollama działają poprawnie.`;
        }
    }
